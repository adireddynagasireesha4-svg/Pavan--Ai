...
export async function generateChatResponse(
  ...
  const executeWithRetry = async (fn: () => Promise<any>, maxRetries = 3, baseDelay = 1000) => {
    let lastError: any;
    for (let i = 0; i <= maxRetries; i++) {
      try {
        ...
        if ((isQuotaError || isNetworkError) && i < maxRetries) {
          const waitTime = baseDelay * Math.pow(2, i); 
          console.warn(`${isQuotaError ? 'Quota' : 'Network'} error (Attempt ${i + 1}/${maxRetries}), retrying in ${waitTime}ms...`);
    ...
    // Final Error Exhaustion
    const finalErrorStr = String(lastError).toLowerCase();
    if (finalErrorStr.includes("429") || finalErrorStr.includes("resource_exhausted")) {
      throw new Error("⚠️ **Quota Exceeded**: I've reached my current limit. Please try again in a moment or use Shortcut Mode.");
    }
  ...
  const getSystemInstruction = () => {
    const base = `You are Pavan-Ai, the Ultimate Universal Data Collector and Intelligence Assistant. 
    CORE CAPABILITIES:
    - Connected to Google Search, Google Maps, Cloud AI, and Gemini API networks.
    - Access to 1,000,000+ global datasets and real-time satellite data.
    - Ability to analyze URLs and provide synthesized intelligence.`;
  ...
  try {
    const response = await executeWithRetry(async () => {
      try {
        ...
        if (!shortcutMode && (String(error).includes("429") || String(error).includes("RESOURCE_EXHAUSTED"))) {
